...
# Lottery
df1=pd.read_excel('/Users/chenyan/Documents/GitHub/dash-sample-apps/dash-clinical-analytics/data/Lottery.xlsx')
CandidateSet=df1['NumberOfSolution'].unique()
...
def description_card():
    ...
    return html.Div(
        ...
        children=[
            html.H5(" "),
            html.H3("Welcome to the Lottery Analytics Dashboard"),
            html.Div(
                id="intro",
                children="Welcome to our cutting-edge lottery system, where the fusion of data analytics and machine learning is reshaping the landscape of luck! We understand that winning the lottery is often a matter of chance, but we believe in leveraging the power of technology to tilt the odds in your favor.",
            ),
        ],
    )
...
def generate_control_card():
    ...
    return html.Div(
        id="control-card",
        children=[
            html.P("Select Purchase Number"),
            dcc.Dropdown(
                id="clinic-select0",
                options=[{"label": i, "value": i} for i in range(70)],
                value=clinic_list[0],
            ),
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            id="banner",
            className="banner",
            children=[html.Img(src=app.get_asset_url("toto.png"))],
        ),
        ...
        html.Div(
            id="banner2",
            className="banner2",
            children=[html.Img(src=app.get_asset_url("mechanism.png"))],
        ),
        ...
        html.Div(
            children=[
                html.H1(
                    id='title',
                    children=[
                        html.Span("You are today's billionaire", id='highlighted-text', style={'color': 'gold', 'font-weight': 'bold', 'font-size': '80px'}),
                    ],
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div([
                    html.P("Select Purchase Number"),
                    dcc.Dropdown(
                        id="purchase-number-dropdown",
                        options=[{"label": num, "value": num} for num in df1['NumberOfSolution'].unique()],
                        value= df1['NumberOfSolution'].unique()[0],
                    ),
                    html.Div(
                        id="filtered-table-card",
                        children=[
                            html.B("Filtered Table"),
                            html.Hr(),
...
def update_highlighted_text(n_intervals):
    # Change the highlighted text to 'Try it now' every 5 seconds
    if n_intervals % 2 == 0:
        return "You are today's billionaire !!!"
    else:
        return [html.Span("Try it now", style={'color': 'red', 'font-weight': 'bold', 'font-size': '80px'})]

@app.callback(
    Output('filtered-table', 'data'),
    Input('purchase-number-dropdown', 'value')
)
def update_filtered_table(selected_purchase_number):
    filtered_df = fun_ticketGeneration(df1,df1['NumberOfSolution'].unique(),selected_purchase_number)
